...
if [ -d "venv" ]; then
    ...
    echo "$(date): No venv/ found. Skipping." >> logs/scheduled_update.log
    exit 1
fi

if python scripts/market_hours.py | grep -q "OPEN"; then
    echo "$(date): Market open — running update." >> logs/scheduled_update.log
    ./scripts/update_hourly.sh >> logs/scheduled_update.log 2>&1
else
    echo "$(date): Market closed — skipping." >> logs/scheduled_update.log
fi
